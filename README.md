...
JavaScriptの責務は `config/architecture-responsibilities.json` に明示し、物理フォルダ名から推測しません。`npm run architecture:check`と`npm run architecture:strict`は循環importと禁止依存が0件であることを検査します。詳細は[ADR 0001](docs/adr/0001-monitoring-architecture-boundaries.md)と[設定ファイル役割監査](docs/config-role-audit.md)を参照してください。
