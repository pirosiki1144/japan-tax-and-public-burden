...
- `config/social-insurance-adapters.yaml` は社会保険料を制度component・負担主体・適用期間・単位・確定区分に分けます。合計率と本人・事業主の内訳には集計関係を持たせ、配布時に同時加算しません。保険者・地域・業種別の値を全国一律値へ置き換えず、網羅前の対象は理由付きで保留します。
- `config/public-burden-adapters.yaml` は税・社会保険料以外の公的負担について、実装済みの負担主体・算定基礎・値・単位・適用期間と、値を安全に確定できない制度の保留理由を管理します。合計と内数を区別し、概要資料にない適用開始日は推測しません。
