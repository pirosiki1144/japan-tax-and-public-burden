...
件数の最終比較と#69の受け入れ条件監査は[リファクタリング最終監査](refactoring-final-audit.md)に記録する。現在値はJSONレポートを参照する。
...
| 薄い委譲、旧経路 | 削除済み | 未使用の`source-adapters.js`と重複設定を削除 | #75完了 |
| fixtures | 維持 | 外部通信なしで公式形式とfail-closedを検証する独立責務 | #74/#75完了 |
