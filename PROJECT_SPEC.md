...
3. 実装と検証はIssueの内容を基準に行う。
4. Pull Requestへ関連Issueと受け入れ条件の結果を記載する。
5. ユーザーによるPR承認を実装内容に対する確認として扱い、承認前に自動マージしない。

Issueの範囲内にある通常の読み取り、編集、検証、Git・GitHubコマンドは、実行ごとの許可確認を不要とする。ただし、破壊的操作、権限拡張、外部への通知、Issueの範囲を実質的に広げる変更は、必要な権限と影響を明示する。
