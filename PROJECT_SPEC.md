...
1. 成立したニュースIssueを削除するか、Closeして保存するか
   推奨はClose。自動削除は監査履歴を失う。
2. 構想段階の報道だけで主状態を「未適用」「適用中（未適用変更あり）」へ変えるか
   推奨は変更レコードを作り、確度を表示した上で主状態へ反映する。ただし一覧表示で確度を必ず併記する。
3. 法令に直接基づかないが実質的に強制される費用をどこまで含めるか
4. 自治体を全国一括で扱うか、代表自治体から段階導入するか
5. 税収・徴収実績の開始年度と粒度
6. リポジトリのライセンス
