...
test("automation is enabled only where an adapter and extraction targets exist", async () => {
  ...
  assert.deepEqual(automated.map(({ tax_id }) => tax_id), ["adverse-drug-reaction-contribution", "consumption-tax", "disability-employment-levy", "educational-public-transmission-compensation", "employment-insurance-premium", "infection-contribution", "pension-insurance-premium", "universal-service-fee"]);
  assert.equal(manual.length, 104);
