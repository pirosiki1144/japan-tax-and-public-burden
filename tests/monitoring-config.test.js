...
test("automation is enabled only where an adapter and extraction targets exist", async () => {
  ...
  assert.deepEqual(automated.map(({ tax_id }) => tax_id), ["consumption-tax", "disability-employment-levy", "educational-public-transmission-compensation", "employment-insurance-premium", "pension-insurance-premium"]);
