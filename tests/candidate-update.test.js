...
const apply = (options) => applyCandidateUpdates({ ...options, writeText: (path, content) => writeFile(path, content, "utf8") });

test("a deterministic candidate updates only its declared canonical target", async () => {
  const root = await fixtureRoot();
  const scan = scanWith([difference()]);
  const result = await apply({ root, scan });
...
test("a repeated candidate is idempotent", async () => {
  const root = await fixtureRoot();
  const scan = scanWith([difference()]);
  await apply({ root, scan });
  const second = await apply({ root, scan });
  assert.equal(second.applied.length, 0);
});

test("source errors, untargeted changes, and stale canonical values stop safely", async () => {
  const root = await fixtureRoot();
  await assert.rejects(apply({ root, scan: { schema_version: 1, status: "error", results: [{ source_id: "bad", status: "error" }] } }), /source errors/);
  await assert.rejects(apply({ root, scan: scanWith([{ ...difference(), target: null }]) }), /human review/);
  await writeFile(join(root, "data/phases/sample.yaml"), "- phase_id: sample-phase\n  value:\n    numeric_value: 8.1\n", "utf8");
  await assert.rejects(apply({ root, scan: scanWith([difference()]) }), /changed since scan/);
});

test("conflicting duplicate targets are rejected before writing", async () => {
  const root = await fixtureRoot();
  await assert.rejects(apply({ root, scan: scanWith([difference(7.9), difference(8.0)]) }), /Conflicting candidates/);
  const document = parse(await readFile(join(root, "data/phases/sample.yaml"), "utf8"));
