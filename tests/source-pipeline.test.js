...
test("all configured automated sources run through one shared pipeline", async () => {
  ...
  assert.equal(result.status, "no_change");
  assert.deepEqual(result.results.map(({ source_id }) => source_id), ["egov-laws", "nta-consumption-tax-rates", "mhlw-employment-insurance-rates", "jeed-disability-employment-levy", "mext-educational-public-transmission-compensation", "nenkin-pension-premiums"]);
});
